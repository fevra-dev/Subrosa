...
| **Sources** | `skills/data-minimization/references/regulatory-reference.md` §Thailand (brief entry) + quick-select row ("Sec. 22 (minimization)"). No Thailand rows exist in the breach/children/rights sub-files — sourcing for this record is a single paragraph plus one table row; cells beyond it are flagged accordingly. |
