...
Statutory citations and jurisdiction facts (deadlines, thresholds, ages, category lists) in this skill and its reference files derive from the normalized taxonomy: `regulatory-taxonomy.md` + the per-jurisdiction records (`regulatory-taxonomy--*.md`, axes A0–A12). Records consumed here: `ca-pipeda-law25`, `eu-gdpr-uk`, `us-ca-ccpa`, `br-lgpd`, `cn-pipl`, `sg-pdpa`, `th-pdpa` (+ COPPA and ePrivacy as sectoral overlays pending their variant profile).
...
For "design-to-strictest" defaults use `regulatory-taxonomy--floor.md`. For known cross-regime incompatibilities (erasure vs immutability, etc.) use `regulatory-taxonomy--conflicts.md`.
