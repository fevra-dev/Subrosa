...
`test_type:` minimum-necessary + explicit excessive-collection prohibition. **Art. 6** (Chinese text sourced in reference file): clear and reasonable purpose; directly related to purpose; method with least impact on personal rights; collection limited to minimum scope; **excessive collection (过度收集) prohibited** — enforcement has cited apps collecting location/contacts beyond stated function. Extends to derived fields: generating behavioral profiles beyond stated purpose violates Art. 6 even with minimized inputs (`skills/data-minimization/SKILL.md` Step 2b rule 5). P-mapping: **P1** (stricter than GDPR, per source). Tag: `ARCH-DISSOLVES`.
