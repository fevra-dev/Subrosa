...
- Designing to one global standard → `regulatory-taxonomy--floor.md` (strictest-regime-wins)
- Obligations collide across regimes (erasure vs immutability, logging vs storage limitation) → `regulatory-taxonomy--conflicts.md`
- Arguing architecture over policy → `regulatory-taxonomy--arch-rollup.md` (which obligations dissolve under selective disclosure)
