...
| C2 mandated logging vs storage limitation | Field-level tokenization inside logs; tiered TTLs (see `SKILL.md` §Privacy-Preserving Audit Logs; ZK compliance attestation at the frontier) |
...
Regulatory mapping (from records — not freetext):
  ...
  FLOOR      taxonomy/regulatory-taxonomy--floor.md A9 row satisfied for all listed markets.
```
