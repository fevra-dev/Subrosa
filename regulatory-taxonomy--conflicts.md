...
**Mechanism:** the statutes assume deletion is physically possible; a public ledger makes it impossible post-write. The suite's canonical collision between privacy-by-architecture and privacy-by-policy (`privacy-suite.md` §Blockchain Continuity).
**Resolution (sourced):** personal data fully off-chain; only content-addressed commitments on-chain; where on-chain payloads are unavoidable, encrypt-before-write and treat key destruction as practical erasure; the suite's blockchain caveat clause (`consent-language--rights-language.md`) discloses the residue.
...
**Resolution (sourced):** tiered retention (hot/warm/cold) with sensitivity-graded TTLs on log *fields*, pseudonymized identity fields in logs, and a documented MINIMIZATION EXCEPTION RECORD per retained high-fidelity field (`data-minimization--regulatory-reference.md` template).
...
**Arch reading:** `ARCH-DISSOLVES` under local-only/edge processing; PSI/MPC/HE let global functions run without the data crossing (`privacy-architecture.md`).
...
**Resolution (sourced + architectural):** ephemeral verification — the sourced VPC method is "check photo ID **+ delete afterward**"; verify, don't retain. Architectural end-state: ZKP age/guardianship predicate proofs (`privacy-architecture.md`) — prove "parent of user, over 18" with zero documents retained.
...
**Resolution:** already built into the statutes — Art. 17(3)(b) exception, CCPA deletion exceptions; restrict processing to what the exception permits and disclose the exception in the notice (`consent-language--rights-language.md` retention-exceptions clause).
