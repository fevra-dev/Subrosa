...
> **Source of truth:** GPC legal-status and consent-standard anchors derive from `regulatory-taxonomy.md` records (`eu-gdpr-uk`, `us-ca-ccpa`); the ePrivacy Directive itself is a sectoral overlay pending its variant profile (see master §Design Contract 4). Reconciled 2026-07-04; log: `.fable/reconciliation-log.md`. On conflict, the record wins (see `consent-language.md` §Regulatory Source of Truth).
