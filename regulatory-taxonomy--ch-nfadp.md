...
## A12 — DPIA / PIA
`dpia_model:` mandatory-triggered — **Art. 22** DPIA where processing likely presents high risk to personality/fundamental rights (+ Ordinance Art. 14); **Art. 23** — consult FDPIC before proceeding where the DPIA shows unmitigable high residual risk; **Art. 23(4)** — appointing a DPO exempts from the FDPIC notification duty. *(Primary-source corrected 2026-07-05: the suite's "Art. 12 registration" claim conflated two provisions — Art. 12 is the **register of processing activities** (>250 employees or risky processing; parked for the v1.1 governance field), and the notification-unless-DPO mechanic is Art. 23(4).)* Tag: `PROCEDURAL`.
