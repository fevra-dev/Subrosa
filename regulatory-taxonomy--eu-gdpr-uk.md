...
| **Sources** | `data-minimization--regulatory-reference.md` · `data-minimization--pipeda-gdpr.md` · `consent-language.md` · `consent-language--rights-language.md` · `consent-language--breach-notification.md` · `consent-language--childrens-consent.md` · `consent-language--cookie-consent.md` · `privacy-impact-assessment--dpia-triggers.md` |
...
- `basis_model:` **enumerated-bases** — Art. 6(1): consent (a), contract (b), legal obligation (c), legitimate interests (f) (bases as used across `consent-language.md`; the vital-interests and public-task bases are not exercised in suite sources — *not located there*, standard Art. 6(1)(d),(e) `[UNVERIFIED]`).
...
- Escalation rule (sourced): any Art. 9 field is CRITICAL tier regardless of context. **Derived/inferred data counts** — a model inferring an Art. 9 attribute produces special-category data even if none was collected (per `data-minimization.md` Step 2b).

Tag: `ARCH-DISSOLVES` where the attribute is proven, not disclosed (ZKP predicate proofs, anonymous credentials — `privacy-architecture.md`); otherwise `PROCEDURAL` (explicit-consent mechanics).
...
- Notice duties feeding rights: Arts. 13/14 (full checklist in `consent-language.md` Step 4).
...
- **Art. 8:** consent-based processing lawful from 16; member states may lower to 13. Member-state table in `consent-language--childrens-consent.md` (13: Nordics/Baltics et al.; 14: AT/HU/IT/ES; 15: FR; 16: DE/IE/NL et al.).
...
- Prior consultation: Art. 36 when residual risk high `[UNVERIFIED — Art. 36 named in privacy-suite.md description ("Art. 36 prior consultation trigger") but mechanics not detailed in sources]`.
